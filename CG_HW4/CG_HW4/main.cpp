...
void initTextures(int index)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB,
		IH[index].Width, IH[index].Height, 0,
		GL_RGB, GL_UNSIGNED_BYTE, image[index]);
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	switch(key){
		...
		case 'M':
			MAG_F = !MAG_F;
			if (MAG_F) printf(" GL_TEXTURE_MAG_FILTER, GL_NEAREST \n");
			else printf(" GL_TEXTURE_MAG_FILTER, GL_LINEAR \n");
			break;
		case 'm':
			MIN_F = !MIN_F;
			if (MIN_F) printf(" GL_TEXTURE_MIN_FILTER, GL_NEAREST \n");
			else printf(" GL_TEXTURE_MIN_FILTER, GL_LINEAR \n");
			break;
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("CS550000 CGHW4 103060017");
