...
void main() {
	...
	gl_FragColor = vec4(vv4vertice.zzz/2.0/sqrt(3.0)+0.5, 0.0);
	vec4 v4texColor = texture2D(us2dtexture, v2texCoord).bgra;
	...
	if(PV_FLAG == 0)
		gl_FragColor = 0.1 * color + 0.9 * v4texColor;
	else
		gl_FragColor = 0.1 * vv4color + 0.9 * v4texColor;

	if (uiisTextureMapping == 0 && PV_FLAG == 0){
		gl_FragColor = color;
	}
	else if (uiisTextureMapping == 0 && PV_FLAG == 1){
